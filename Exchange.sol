	...
	ReentrancyGuardUpgradeable
{
	...
	function createToken(
	...
	) external payable nonReentrant returns (ERC20) {
		require(msg.value >= createFee, "Insufficient payment");

		ERC20 newToken = new Token(name, symbol, 1000000000);
		...
		sendProtocolFeeToTreasure(createFee);
	...
	function createTokenWithBuy(
		...
		require(msg.value > createFee, "Insufficient payment");
		uint256 firstBalance = msg.value - createFee;
		...
		uint256 receivedAmount = (firstBalance * 10000) /
			(10000 + commissionRate);
		...
		uint256 fee = firstBalance - receivedAmount + createFee;
		...
		if (reserveBalanceAfterTxCompleted < stopBalance) {
			...
			require(
				firstBalance >= (fee + receivedAmount),
				"insufficient payment"
			);
			refund = firstBalance - (fee + receivedAmount);
		}
